...
<!-- Cloudflare Web Analytics --><script defer src='https://static.cloudflareinsights.com/beacon.min.js' data-cf-beacon='{"token": "de14f379a11445ada5285bb32a54af97"}'></script><!-- End Cloudflare Web Analytics -->
    ...
    #response1, #response2, #response3, #response4 {
      background-color:#0668FD;border-radius:8px;border:none;padding:12px;color:white;font-size:1em;
    }
        html,
        body {
          height: 100%;
        ...
        * {
        ...
        #response1, #response2, #response3, #response4 { margin: 4px; }
...
<?php } else { ?>
 ...
 <!-- Cloudflare Web Analytics --><script defer src='https://static.cloudflareinsights.com/beacon.min.js' data-cf-beacon='{"token": "de14f379a11445ada5285bb32a54af97"}'></script><!-- End Cloudflare Web Analytics -->
     ...
     <script>
      <?php if($_GET['learn']=='true'){ ?> var learnmode = true; <?php } else { ?> var learnmode = false; <?php } ?>
 var questions = [<?php $words = $thisClass->query("SELECT * FROM Quiz".$setID);
 if($words->num_rows > 0){
   while($row = $words->fetch_assoc()){ echo '`'.str_replace("\\","\\\\",$row["Question"]).'`,'; }} ?>""];
 var c1s = [<?php $words = $thisClass->query("SELECT * FROM Quiz".$setID);
 if($words->num_rows > 0){
   while($row = $words->fetch_assoc()){ echo '`'.str_replace("\\","\\\\",$row["C1"]).'`,'; }} ?>""];

   var ic1s = [<?php $words = $thisClass->query("SELECT * FROM Quiz".$setID);
   if($words->num_rows > 0){
     while($row = $words->fetch_assoc()){ echo '`'.str_replace("\\","\\\\",$row["Ic1"]).'`,'; }} ?>""];
     var ic2s = [<?php $words = $thisClass->query("SELECT * FROM Quiz".$setID);
     if($words->num_rows > 0){
       while($row = $words->fetch_assoc()){ echo '`'.str_replace("\\","\\\\",$row["Ic2"]).'`,'; }} ?>""];
       var ic3s = [<?php $words = $thisClass->query("SELECT * FROM Quiz".$setID);
       if($words->num_rows > 0){
         while($row = $words->fetch_assoc()){ echo '`'.str_replace("\\","\\\\",$row["Ic3"]).'`,'; }} ?>""];
 questions.pop();
 c1s.pop();
 ic1s.pop();
 ic2s.pop();
 ...
 class="fa-solid fa-arrow-right-to-bracket"></i></button>
     ...
     <script src='https://www.quizza.org/static/scripts/mcQuiz.js'></script>
 </body>
